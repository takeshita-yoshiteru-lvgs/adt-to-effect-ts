...
class InfraRedRemoteController extends Context.Tag("InfraRedRemoteController")<
  InfraRedRemoteController,
  {
    sendCommand(command: string): Effect.Effect<void, "ACNetworkError">;
  }
>() {}

// Service Layer
class UserSettingService extends Context.Tag("UserSettingService")<
  UserSettingService,
  {
    getUserSetting(
      userId: number
...
class AirConditioner extends Context.Tag("AirConditioner")<
  AirConditioner,
  {
...
function checkWakeUpTime(
  userSetting: UserSetting
): Effect.Effect<void, "NotWakeUpTime", void> {
  return Effect.gen(function* (_) {
