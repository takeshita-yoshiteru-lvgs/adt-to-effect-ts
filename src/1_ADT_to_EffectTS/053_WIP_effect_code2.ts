...
const flatMapMerge = <A, E, R, A2, Key extends string>(
  key: Key,
  flatMapFunc: (a: A) => Effect.Effect<A2, E, R>
): Effect.Effect<A & { hoge: A2 }, E, R> => {
  return flatMap<A, E, R, A & { hoge: A2 }>((a) =>
    flatMapFunc(a).pipe(map((r) => Object.assign({}, a, { hoge: a })))
  );
};
...
async function purchaseItems(
  ...
  const pp = pipe(
    ...
    map((args) => ({ ...args, price: calculatePrice(user, items) })),
    flatMap((args) =>
      payByCreditCard(args.creditCard, args.price).pipe(
        map((payResult) => ({
          ...args,
          payResult,
        }))
